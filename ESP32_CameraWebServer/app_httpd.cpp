...
httpd_handle_t stream_httpd = NULL;    //网页、拍照、状态、控制
httpd_handle_t camera_httpd = NULL;    //视频流
...
static esp_err_t capture_handler(httpd_req_t *req) {
  ...
  fb = esp_camera_fb_get();              // 从摄像头获取一帧图像or it won't be visible in the frame. A better way to do this is needed.
  enable_led(false);
#else
  fb = esp_camera_fb_get();
#endif
  ...
  httpd_resp_set_type(req, "image/jpeg");   //设置 HTTP response 类型为 image/jpeg
  ...
  if (fb->format == PIXFORMAT_JPEG) {
    ...
    fb_len = fb->len;
#endif
    res = httpd_resp_send(req, (const char *)fb->buf, fb->len);  //用 httpd_resp_send() 把 JPEG bytes 返回给 PC
  } else {
  ...
  esp_camera_fb_return(fb);   //调用 esp_camera_fb_return() 归还 frame buffer
#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
  int64_t fr_end = esp_timer_get_time();
#endif
...
//注册Camera server的URL  还有Stream server的URL
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &capture_uri);    //浏览器或 Python 一访问 /capture，ESP32-CAM 就会调用 capture_handler()
